...
function CSGO({profile, stats}) {
  ...
  return (
    <div>
      ...
      <div className="fixed top-0 h-12 w-full bg-white bg-opacity-30 shadow-xl sm:h-full sm:w-16 z-50">
        <div className="flex items-center h-full sm:flex-col">
          ...
          <div className="flex h-full items-center space-x-6 pl-6 text-white sm:flex-col sm:pl-0 sm:space-x-0 sm:space-y-4 sm:pt-6">
            ...
            <div className="opacity-70 hover:opacity-100">
              <ArchiveIcon onClick={() =>router.push("/inventory")} className="h-[33.75px]" />
            </div>
            <div
      ...
      <CSGOStats profile={profile} stats={stats} />
    </div>
  );
}

function CSGOStats({ profile, stats }) {
  const openProfile = () => {
  ...
  return (
    <div className="relative grid grid-cols-1 text-white top-12 sm:ml-16 lg:grid-cols-2">
      <div className="m-6 p-10 font-desktop text-2xl flex flex-col space-y-7 bg-white bg-opacity-20 rounded-md shadow-lg sm:m-10">
        <p>
          {
            "There is a hidden gamer in all of us. Some of us plays single player games like Assassin's Creed or The Witcher."
          }
        </p>
        <p>
          {" "}
          I am also a player myself and i usually play competitive games. One of
          my favorite competitive games, <br />{" "}
